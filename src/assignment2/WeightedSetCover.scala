...
object WeightedSetCover extends App {
  ...
  var w = new Array[Int](m) // Cost of every set.
  ...
  var x: Array[Int] = new Array[Int](m) // Decision variables.
  var lb: Double = 0.0 // Lower bound on the objective.

  var bestFeasibleX: Array[Int] = Array.fill[Int](m)(1)
  var bestCost: Int = w.sum // Best-known lower bound objective; we initialize it to the sum of all elements.
  ...
  def lowerBound(): Double = {
    ...
    for (k <- 1 to maxiter) {
      ...
      for (j <- lambda.indices) {
        var mul: Int = 1
        for (i <- setsContaining(j) if x(i) == 1) mul -= 1 // Find the sets in which element j appears.
      ...
      for (i <- coeff.indices) {
        ...
        if (coeff(i) < 0.0) x(i) = 1 // Decide which sets to take.
      }
      ...
      if (isFeasible()) {
        ...
        if (xCost < bestCost) {
          bestCost = xCost
          bestFeasibleX = copy(x) // Copy is needed because x can still change.
        }
      }
    }
    lb
   ...
   * @return the cost of x.
   */
  def cost(): Int = {
    var co: Int = 0
    for (i <- x.indices if x(i) == 1) co += w(i)
    co
  }
  ...
  def isFeasible(): Boolean = {
    ...
    for (i <- x.indices if x(i) == 1; j <- elementsIn(i)) elemsTaken(j) = 1 // Set values to 1 when element is taken.
